"""
Test script for WSI ACMIL MLP using k-fold ensemble.

Evaluates both internal and external test sets using a 10-fold ensemble of
ACMILClassifier models (head='mlp', GELU + LayerNorm slide classifier).

Usage:
    python test_wsi_acmil.py --run_dir <path_to_run> --output_dir <output_dir> [--gpu_id 0]
"""
...
def get_args():
    p = argparse.ArgumentParser(description='Test WSI ACMIL MLP (k-fold ensemble).')
    p.add_argument('--run_dir', type=str, required=True)
    p.add_argument('--output_dir', type=str, default='./output_test_wsi_acmil')
...
# ---------------------------------------------------------------------------
# Inference  (ACMIL: forward returns sub_preds, slide_pred, attn_logits)
# ---------------------------------------------------------------------------
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            for m in models_list:
                _, slide_pred, _ = m(feats, mask)
                prob = torch.sigmoid(slide_pred).cpu().item()
...
def main():
    ...
    # Determine head type from HP (presence of classifier_dropout implies MLP)
    ...
    print(f'ACMIL head: {head}')

    def model_fn():
        return ACMILClassifier(
            input_dim=input_dim,
            D_inner=D_inner,
            D_attn=D_attn,
            n_token=n_token,
