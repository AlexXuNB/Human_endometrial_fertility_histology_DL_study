"""
Test script for WSI ResNet-18 MIL using k-fold ensemble.

Each WSI slide's prediction is the mean sigmoid probability across all patches.
Evaluates both internal WSI test set and external test set.

Usage:
    python test_wsi_resnet18.py --run_dir <path_to_run> --output_dir <output_dir> [--gpu_id 0]
"""
...
def get_args():
    p = argparse.ArgumentParser(description='Test WSI ResNet-18 MIL (k-fold ensemble).')
    p.add_argument('--run_dir', type=str, required=True)
    p.add_argument('--output_dir', type=str, default='./output_test_wsi_resnet18')
...
def infer_slide_tensor(model, patch_tensor, device, batch_size):
    """Run model on a pre-loaded patch tensor. Avoids re-reading patches from disk per fold."""
    all_probs = []
    with torch.no_grad():
        for i in range(0, len(patch_tensor), batch_size):
            imgs = patch_tensor[i:i + batch_size].to(device)
            logits = model(imgs)
...
def run_inference(df, patches_dir, fold_info, dropout, device, batch_size, num_workers=4):
    ...
    print(f'  Running inference on {len(df)} slides...')
    ...
    for idx, (_, row) in enumerate(df.iterrows()):
        ...
        # Load patches from disk once, then reuse for all fold models.
        dataset = SlideDataset(slide_dir, transform=EVAL_TRANSFORM)
        if len(dataset) == 0:
            print(f'  Warning: no patches for slide {slide_id}, skipping')
            continue
        loader = DataLoader(dataset, batch_size=batch_size, shuffle=False,
                            num_workers=num_workers, pin_memory=False)
        patch_tensor = torch.cat([b for b in loader], dim=0).cpu()  # (N, 3, 224, 224)

        if (idx + 1) % 5 == 0 or idx == 0:
            print(f'  Slide {idx + 1}/{len(df)}: {slide_id} ({len(patch_tensor)} patches)')

        fold_probs = [infer_slide_tensor(m, patch_tensor, device, batch_size)
...
def main():
    ...
    test_patches_dir = hp.get('test_patches_dir',
        'data/wsi_patches_224px/internal_testing')
    ext_patches_dir = hp.get('ext_patches_dir',
        'data/wsi_patches_224px/external_testing')
    ...
    slide_ids, labels, probs, f_ids, thresholds = run_inference(
        test_df, test_patches_dir, fold_info, dropout, device, args.batch_size, args.num_workers)
    ...
    slide_ids_e, labels_e, probs_e, f_ids_e, thresholds_e = run_inference(
        ext_df, ext_patches_dir, fold_info, dropout, device, args.batch_size, args.num_workers)
