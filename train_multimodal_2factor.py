"""
Training script for Multimodal 2-Factor OOF Logistic Regression (WSI + LE).

This script trains a meta-learner on top of two pre-trained fold models:
  - WSI model: PoolingClassifier trained by train_wsi_pooling.py
  - LE  model: PoolingClassifier trained by train_le_pooling.py

Algorithm (Out-Of-Fold logistic regression):
  1. For each of K folds, load the fold model (NOT seen the validation samples).
  2. Run inference on the held-out validation samples to collect OOF predictions.
  3. Concatenate (wsi_oof_prob, le_oof_prob) as a 2-D feature vector.
  4. Train  LogisticRegression(solver='liblinear', class_weight='balanced') on these features.
  5. Evaluate on internal test set and external test set using mean-ensemble
     probabilities from both modalities fed into the logreg meta-learner.
...
def _find_best_model(fold_dir):
    """Return path to best checkpoint inside fold_dir."""
...
def load_pooling_model(fold_dir, hp, device):
    """Load a PoolingClassifier from a fold directory."""
...
def predict_probs(model, loader, device):
    """Return {sample_id: prob} and {sample_id: label} dicts."""
    probs, labels = {}, {}
...
def build_oof_predictions(run_dir, hp, feats_dir, k_folds, num_workers, device):
    """Collect OOF probabilities by running each fold model on its held-out samples."""
...
def ensemble_predict(run_dir, hp, dataset_csv, split_name, feats_dir,
    ...
    # Accept both 'testing' and 'test' as the same split
    actual = split_name
    ...
    if not all_fold_probs:
        print('  No models loaded; cannot generate ensemble predictions.')
        return {}, labels

    # Mean ensemble across folds
...
def train_logreg(oof_wsi_probs, oof_le_probs, labels):
    rows = []
    for sid, y in labels.items():
        if sid in oof_wsi_probs and sid in oof_le_probs:
    ...
    X = df[['wsi_prob', 'le_prob']].values
    y = df['label'].values

    # Raw LogisticRegression — no StandardScaler, matching reference training.
    ...
    print('\n' + '=' * 40)
    print('LOGREG MODEL WEIGHTS (WSI + LE)')
    print('=' * 40)
    for name, w in zip(['wsi_prob', 'le_prob'], coefs):
        print(f'  {name}: {w:.4f}')
...
def main():
    ...
    if args.run_name is None:
        ...
        args.run_name = f'oof_logreg_{ts}'
    ...
    # ------------------------------------------------------------------
    # Part 1: Build OOF predictions and train logistic regression
    # ------------------------------------------------------------------
    print('\n' + '=' * 80)
    print('PART 1: OOF PREDICTIONS FOR LOGISTIC REGRESSION')
    print('=' * 80)
    ...
    logreg, oof_df = train_logreg(wsi_oof_probs, le_oof_probs, labels)

    oof_df['combined_prob'] = logreg.predict_proba(
        oof_df[['wsi_prob', 'le_prob']].values)[:, 1]
    ...
    oof_metrics['optimal_threshold'] = 0.5
    ...
    # ------------------------------------------------------------------
    # Part 2: Evaluate on internal test and external test sets
    # ------------------------------------------------------------------
    test_splits = [
        ('testing', args.wsi_dataset_csv, args.wsi_test_feats_dir,
         args.le_dataset_csv, args.le_test_feats_dir),
        ('ext_test', args.wsi_ext_dataset_csv, args.wsi_ext_test_feats_dir,
         args.le_ext_dataset_csv, args.le_ext_test_feats_dir),
    ]

    for split_name, wsi_csv, wsi_feats, le_csv, le_feats in test_splits:
        print('\n' + '=' * 80)
        ...
        X = test_df[['wsi_prob', 'le_prob']].values
    ...
    print('\n' + '=' * 80)
    print('MULTIMODAL 2-FACTOR OOF LOGREG COMPLETE')
    print('=' * 80)
