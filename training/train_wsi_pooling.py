"""
Training script for WSI-level UNI2-h global mean pooling classification with k-fold CV.

Model variants (selected via --head):
  - linear: Global mean pooling -> Linear(1536, 1)
  - mlp:    Global mean pooling -> MLP (1536 -> 768 -> 384 -> 1) with LayerNorm + GELU

Input: Pre-extracted UNI2-h patch features (1536-d) stored in H5 files.
...
class H5SlideDataset(Dataset):
    """Loads pre-extracted patch features from H5 files."""
...
def train_fold(args, train_df, val_df, test_df, ext_test_df, device, fold_id):
    ...
    feats = {
        'train': args.train_feats_dir,
        'val': args.train_feats_dir,
        'test': args.test_feats_dir,
        'ext_test': args.ext_test_feats_dir,
    }
    ...
    model = PoolingClassifier(
        input_dim=args.input_dim, head=args.head,
        dropout=args.dropout, pooling=args.pooling
    ).to(device)
    print(f'Model: PoolingClassifier (head={args.head}, pooling={args.pooling})')
...
def get_args():
    p = argparse.ArgumentParser(
        description='WSI UNI2-h global pooling classification with k-fold CV.')
